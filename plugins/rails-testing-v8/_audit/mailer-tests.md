...
| `assert_enqueued_email_with UserMailer, :create_invite, args: […]` positional-args example (inline comment absorbs the `args: [{ from:, to: }]` keyword-arg variant) | `guides/source/testing.md` lines 2240–2277 |
| `params:` + `args:` parameterized-mailer example using `UserMailer.with(all: "good")`; trailing NOTE absorbs the `UserMailer.with(to: …)` alternative form | `guides/source/testing.md` lines 2279–2317 |
